...
async function loadDashboardData() {
    try {
        ...
        document.getElementById("totalUsers").textContent = stats.totaleDipendenti || 0;
        document.getElementById("activeEmployees").textContent = stats.totaleDipendenti || 0;
        document.getElementById("todayPresences").textContent = stats.presenzeOggi || 0;
        document.getElementById("pendingRequests").textContent = stats.richiesteInAttesa || 0;

        // Load today's presences
        const presenzeResponse = await fetch("http://localhost:3000/api/presenze/oggi");
...
function updateRolesChart(users) {
    ...
    const roleCounts = {};
    users.forEach(user => {
        roleCounts[user.LivelloAccesso] = (roleCounts[user.LivelloAccesso] || 0) + 1;
    });
        ...
        <tr>
            ...
            <td class="px-6 py-4 whitespace-nowrap">
                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                    user.LivelloAccesso === 'Admin' ? 'bg-red-100 text-red-800' :
                    user.LivelloAccesso === 'Responsabile' ? 'bg-yellow-100 text-yellow-800' :
...
function displayEmployees(employees) {
    ...
    tbody.innerHTML = employees.map(employee => `
        <tr>
            ...
            <td class="px-6 py-4 whitespace-nowrap">
                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                    employee.Attivo ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                }">
...
async function updateRequestStatus(requestId, status) {
    try {
        const response = await fetch(`http://localhost:3000/api/richieste/${requestId}`, {
            method: "PUT",
...
async function loadStatistics() {
    ...
    try {
        ...
        const response = await fetch(`http://localhost:3000/api/presenze/statistiche?mese=${monthNum}&anno=${year}`);
...
function displayStatistics(stats) {
    ...
    if (stats.length === 0) {
        container.innerHTML = '<p class="text-gray-500">Nessun dato disponibile per questo periodo</p>';
        return;
    }
...
function exportReport() {
    ...
    showToast(`Report ${reportType} esportato per il periodo ${startDate} - ${endDate}`);
}
