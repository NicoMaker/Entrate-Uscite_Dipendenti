...
async function loadAttendanceStatus() {
  try {
    const response = await fetch(
      `http://localhost:3000/api/presenze/dipendente/${currentUser.id}?mese=${new Date().getMonth() + 1}&anno=${new Date().getFullYear()}`,
    );
...
async function loadMonthlyStats() {
  try {
    ...
    const response = await fetch(
      `http://localhost:3000/api/presenze/statistiche?mese=${currentMonth}&anno=${currentYear}`,
    );
    ...
    if (userStats) {
      document.getElementById("presenceDays").textContent =
        userStats.giorni_presenza || 0;
      document.getElementById("totalHours").textContent =
        `${((userStats.media_ore_giorno || 0) * (userStats.giorni_presenza || 0)).toFixed(1)}h`;
      document.getElementById("avgHours").textContent =
        `${(userStats.media_ore_giorno || 0).toFixed(1)}h`;
    } else {
...
async function loadRecentActivity() {
  try {
    const response = await fetch(
      `http://localhost:3000/api/presenze/dipendente/${currentUser.id}`,
    );
    const presenze = await response.json();
    ...
    container.innerHTML = recentPresenze
      .map(
        (presenza) => `
...
async function loadAttendanceHistory() {
  ...
  try {
    ...
    const response = await fetch(
      `http://localhost:3000/api/presenze/dipendente/${currentUser.id}?mese=${monthNum}&anno=${year}`,
    );
    const presenze = await response.json();
...
async function loadRequests() {
  try {
    ...
    const userRequests = requests.filter(
      (r) => r.ID_Dipendente === currentUser.id,
    );
...
function showCreateRequestModal() {
  ...
  document.getElementById("requestStartDate").value = tomorrow
    .toISOString()
    .slice(0, 10);
  document.getElementById("requestEndDate").value = tomorrow
    .toISOString()
    .slice(0, 10);
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("attendanceMonth").value = new Date()
    .toISOString()
    .slice(0, 7);
});
